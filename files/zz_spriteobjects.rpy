
translate spanish strings:

    # game/zz_spriteobjects.rpy:703
    old "Do you like my ponytail, [player]?"
    new "¿Te gusta mi coleta, [player]?"

    # game/zz_spriteobjects.rpy:733
    old "Feels nice to let my hair down..."
    new "Sienta bien soltarse el pelo..."

    # game/zz_spriteobjects.rpy:761
    old "Looks cute, don't you think?"
    new "Se ve mona, ¿no crees?"

    # game/zz_spriteobjects.rpy:1140
    old "TALE AS OLD AS TIME"
    new "UNA HISTORIA ANCESTRAL"

    # game/zz_spriteobjects.rpy:1140
    old "TRUE AS IT CAN BE"
    new "CIERTA Y REAL"

    # game/zz_spriteobjects.rpy:1279
    old "Pink Diamond"
    new "Diamante Rosa"

    # game/zz_spriteobjects.rpy:1328
    old "Ze~"
    new "Ze..."

    # game/zz_spriteobjects.rpy:1328
    old "Tea time, tea time. Even if we have coffee, it's tea time. Ehehe~"
    new "Hora del té, hora del té. Aunque tomemos café, es la hora del té. Je, je, je."

    # game/zz_spriteobjects.rpy:1328
    old "Eye of newt, toe of frog..."
    new "Ojo de tritón, pata de rana..."

    # game/zz_spriteobjects.rpy:1328
    old "Now where did I leave that broom..."
    new "A ver dónde he dejado esa escoba..."

    # game/zz_spriteobjects.rpy:1413
    old "My heart beats for you, [player]~"
    new "Mi corazón late por ti, [player]."

    # game/zz_spriteobjects.rpy:1413
    old "Full of love, just like you~"
    new "Lleno de amor, igual que tú."

    # game/zz_spriteobjects.rpy:1441
    old "Cute!"
    new "¡Qué mono!"

    # game/zz_spriteobjects.rpy:1468
    old "Ready to deck the halls, [player]?"
    new "¿Listo para decorar la casa, [player]?"

    # game/zz_spriteobjects.rpy:1638
    old "That's pretty formal, [player]."
    new "Eso es bastante formal, [player]."

    # game/zz_spriteobjects.rpy:1638
    old "Are we going somewhere special, [player]?"
    new "¿Vamos a algún sitio especial, [player]?"

    # game/zz_spriteobjects.rpy:1671
    old "Very versatile."
    new "Muy versátil."

    # game/zz_spriteobjects.rpy:1671
    old "This goes with so many different outfits!"
    new "¡Esto pega con un montón de conjuntos diferentes!"

    # game/zz_spriteobjects.rpy:1704
    old "So cute!"
    new "¡Qué mono!"

    # game/zz_spriteobjects.rpy:1754
    old "Like the ocean..."
    new "Como el océano..."

    # game/zz_spriteobjects.rpy:1787
    old "I love that color!"
    new "¡Me encanta ese color!"

    # game/zz_spriteobjects.rpy:1787
    old "Lavender is a nice change of pace."
    new "El lavanda es un buen cambio de aires."

    # game/zz_spriteobjects.rpy:1820
    old "I've always loved this color..."
    new "Siempre me ha encantado este color..."

    # game/zz_spriteobjects.rpy:1820
    old "It's just like my eyes!"
    new "¡Es igual que mis ojos!"

    # game/zz_spriteobjects.rpy:1852
    old "Do you miss my old ribbon, [player]?"
    new "¿Echas de menos mi antiguo lazo, [player]?"

    # game/zz_spriteobjects.rpy:1852
    old "Back to the classics!"
    new "¡De vuelta a los clásicos!"

    # game/zz_spriteobjects.rpy:1885
    old "Like a warm, rainy day..."
    new "Como un día cálido y lluvioso..."

    # game/zz_spriteobjects.rpy:1885
    old "That's a really unique color, [player]."
    new "Es un color realmente único, [player]."

    # game/zz_spriteobjects.rpy:1918
    old "That's a lovely color!"
    new "¡Es un color precioso!"

    # game/zz_spriteobjects.rpy:1918
    old "Green, just like my eyes!"
    new "¡Verde, igual que mis ojos!"

    # game/zz_spriteobjects.rpy:1951
    old "This purple looks pretty nice, right [player]?"
    new "Este morado queda bastante bien, ¿verdad, [player]?"

    # game/zz_spriteobjects.rpy:1951
    old "Really has a spring feel to it."
    new "La verdad es que da un aire primaveral."

    # game/zz_spriteobjects.rpy:1984
    old "That's beautiful!"
    new "¡Qué bonito!"

    # game/zz_spriteobjects.rpy:1984
    old "Just like autumn leaves..."
    new "Igual que las hojas de otoño..."

    # game/zz_spriteobjects.rpy:2017
    old "Looks cute, right?"
    new "Se ve mono, ¿verdad?"

    # game/zz_spriteobjects.rpy:2017
    old "Good choice!"
    new "¡Buena elección!"

    # game/zz_spriteobjects.rpy:2047
    old "That's an interesting color, [player]."
    new "Es un color interesante, [player]."

    # game/zz_spriteobjects.rpy:2047
    old "I'm quite fond of it, actually."
    new "La verdad es que me gusta bastante."

    # game/zz_spriteobjects.rpy:2077
    old "Red is a beautiful color!"
    new "¡El rojo es un color precioso!"

    # game/zz_spriteobjects.rpy:2077
    old "Just like roses~"
    new "Como las rosas."

    # game/zz_spriteobjects.rpy:2107
    old "That's a beautiful shade of red."
    new "Es un tono de rojo precioso."

    # game/zz_spriteobjects.rpy:2107
    old "Doesn't it look pretty?"
    new "¿A que se ve bonito?"

    # game/zz_spriteobjects.rpy:2137
    old "Like a clear summer sky..."
    new "Como un cielo despejado de verano..."

    # game/zz_spriteobjects.rpy:2137
    old "Nice choice, [player]!"
    new "¡Buena elección, [player]!"

    # game/zz_spriteobjects.rpy:2167
    old "I like the look of this one."
    new "Me gusta cómo queda este."

    # game/zz_spriteobjects.rpy:2167
    old "I've always loved silver."
    new "Siempre me ha encantado el plateado."

    # game/zz_spriteobjects.rpy:2167
    old "Nice choice, [player]."
    new "Buena elección, [player]."

    # game/zz_spriteobjects.rpy:2198
    old "Looks really summer-y, right?"
    new "Parece muy veraniego, ¿verdad?"

    # game/zz_spriteobjects.rpy:2198
    old "Just like a summer sky."
    new "Igual que un cielo de verano."

    # game/zz_spriteobjects.rpy:2228
    old "That's a great color!"
    new "¡Es un color genial!"

    # game/zz_spriteobjects.rpy:2228
    old "Formal! Are you taking me somewhere special, [player]?"
    new "¡Formal! ¿Me llevas a algún sitio especial, [player]?"

    # game/zz_spriteobjects.rpy:2258
    old "This color reminds me of a nice summer day!"
    new "¡Este color me recuerda a un bonito día de verano!"

    # game/zz_spriteobjects.rpy:2354
    old "Pretty in pink!"
    new "¡Guapa de rosa!"

    # game/zz_spriteobjects.rpy:2354
    old "I'm shining like a diamond~"
    new "Estoy brillando como un diamante."

    # game/zz_spriteobjects.rpy:2430
    old "Ready for school!"
    new "¡Lista para el insti!"

    # game/zz_spriteobjects.rpy:2536
    old "Ah, feels nice without the blazer!"
    new "¡Ah, qué bien se está sin la chaqueta!"

    # game/zz_spriteobjects.rpy:2591
    old "Just an ordinary costume, ~ze."
    new "Solo un disfraz normal y corriente..."

    # game/zz_spriteobjects.rpy:2627
    old "~nya?"
    new "¿Miau?"

    # game/zz_spriteobjects.rpy:2627
    old "n-nya..."
    new "M-miau..."

    # game/zz_spriteobjects.rpy:2627
    old "Nya!"
    new "¡Miau!"

    # game/zz_spriteobjects.rpy:2679
    old "Caught you in my web~"
    new "Te he atrapado en mi red."

    # game/zz_spriteobjects.rpy:2679
    old "Don't be scared~"
    new "No tengas miedo."

    # game/zz_spriteobjects.rpy:2679
    old "Don't worry, I don't bite..."
    new "No te preocupes, no muerdo..."

    # game/zz_spriteobjects.rpy:2716
    old "Merry Christmas!"
    new "¡Feliz Navidad!"

    # game/zz_spriteobjects.rpy:2716
    old "What kind of {i}presents{/i} do you want?"
    new "¿Qué tipo de {i}regalos{/i} quieres?"

    # game/zz_spriteobjects.rpy:2716
    old "Happy holidays!"
    new "¡Felices fiestas!"

    # game/zz_spriteobjects.rpy:2752
    old "Would you like to open your present?~"
    new "¿Quieres abrir tu regalo?"

    # game/zz_spriteobjects.rpy:2752
    old "Open your present, ehehe~"
    new "Abre tu regalo, je, je, je."

    # game/zz_spriteobjects.rpy:2752
    old "All I want for Christmas is you~"
    new "Todo lo que quiero por Navidad eres tú."

    # game/zz_spriteobjects.rpy:2752
    old "Santa baby~"
    new "Santa baby."

    # game/zz_spriteobjects.rpy:2752
    old "What {i}else{/i} do you want to unwrap?~"
    new "¿Qué {i}más{/i} quieres desenvolver?"

    # game/zz_spriteobjects.rpy:2794
    old "Very formal!"
    new "¡Muy formal!"

    # game/zz_spriteobjects.rpy:2794
    old "Any special occasion, [player]?"
    new "¿Alguna ocasión especial, [player]?"

    # game/zz_spriteobjects.rpy:2832
    old "Are we going anywhere special today, [player]?"
    new "¿Vamos a algún sitio especial hoy, [player]?"

    # game/zz_spriteobjects.rpy:2832
    old "I've always loved this outfit..."
    new "Siempre me ha encantado este conjunto..."

    # game/zz_spriteobjects.rpy:2864
    old "Do you like what you see, [player]?"
    new "¿Te gusta lo que ves, [player]?"

    # game/zz_spriteobjects.rpy:2926
    old "No U! Ehehe~"
    new "¡Tú más! Je, je, je."

    # game/zz_spriteobjects.rpy:2926
    old "Ready to draw some more cards?~"
    new "¿Listo para robar más cartas?"

    # game/zz_spriteobjects.rpy:2926
    old "Colorful!"
    new "¡Colorido!"

    # game/zz_spriteobjects.rpy:2926
    old "Plus 10 to luck~"
    new "Más 10 de suerte."

    # game/zz_spriteobjects.rpy:2926
    old "Up for a game, [player]?"
    new "¿Te apetece una partida, [player]?"

    # game/zz_spriteobjects.rpy:703
    old "Ponytail"
    new "Coleta"

    # game/zz_spriteobjects.rpy:733
    old "Down"
    new "Suelto"

    # game/zz_spriteobjects.rpy:761
    old "Down (Tied strand)"
    new "Suelto (Mechón atado)"

    # game/zz_spriteobjects.rpy:791
    old "Braided"
    new "Trenzado"

    # game/zz_spriteobjects.rpy:1109
    old "Thermos (Just Monika)"
    new "Termo (Just Monika)"

    # game/zz_spriteobjects.rpy:1140
    old "Rose"
    new "Rosa"

    # game/zz_spriteobjects.rpy:1248
    old "Golden Music Note"
    new "Nota musical dorada"

    # game/zz_spriteobjects.rpy:1328
    old "Witch Hat"
    new "Sombrero de bruja"

    # game/zz_spriteobjects.rpy:1413
    old "Hairclip (Gray hearts)"
    new "Pasador (Corazones grises)"

    # game/zz_spriteobjects.rpy:1441
    old "Hairclip (Pink diamonds)"
    new "Pasador (Diamantes rosas)"

    # game/zz_spriteobjects.rpy:1468
    old "Hairclip (Holly)"
    new "Pasador (Acebo)"

    # game/zz_spriteobjects.rpy:1638
    old "Ribbon (Black)"
    new "Lazo (Negro)"

    # game/zz_spriteobjects.rpy:1671
    old "Ribbon (Black/gray)"
    new "Lazo (Negro/gris)"

    # game/zz_spriteobjects.rpy:1704
    old "Ribbon (Black/pink)"
    new "Lazo (Negro/rosa)"

    # game/zz_spriteobjects.rpy:1754
    old "Ribbon (Blue)"
    new "Lazo (Azul)"

    # game/zz_spriteobjects.rpy:1787
    old "Ribbon (Dark Purple)"
    new "Lazo (Morado oscuro)"

    # game/zz_spriteobjects.rpy:1820
    old "Ribbon (Emerald)"
    new "Lazo (Esmeralda)"

    # game/zz_spriteobjects.rpy:1852
    old "Ribbon (White)"
    new "Lazo (Blanco)"

    # game/zz_spriteobjects.rpy:1885
    old "Ribbon (Gray)"
    new "Lazo (Gris)"

    # game/zz_spriteobjects.rpy:1918
    old "Ribbon (Green)"
    new "Lazo (Verde)"

    # game/zz_spriteobjects.rpy:1951
    old "Ribbon (Light Purple)"
    new "Lazo (Morado claro)"

    # game/zz_spriteobjects.rpy:1984
    old "Ribbon (Peach)"
    new "Lazo (Melocotón)"

    # game/zz_spriteobjects.rpy:2017
    old "Ribbon (Pink)"
    new "Lazo (Rosa)"

    # game/zz_spriteobjects.rpy:2047
    old "Ribbon (Platinum)"
    new "Lazo (Platino)"

    # game/zz_spriteobjects.rpy:2077
    old "Ribbon (Red)"
    new "Lazo (Rojo)"

    # game/zz_spriteobjects.rpy:2107
    old "Ribbon (Ruby)"
    new "Lazo (Rubí)"

    # game/zz_spriteobjects.rpy:2137
    old "Ribbon (Sapphire)"
    new "Lazo (Zafiro)"

    # game/zz_spriteobjects.rpy:2167
    old "Ribbon (Silver)"
    new "Lazo (Plateado)"

    # game/zz_spriteobjects.rpy:2198
    old "Ribbon (Teal)"
    new "Lazo (Verde azulado)"

    # game/zz_spriteobjects.rpy:2228
    old "Ribbon (Wine)"
    new "Lazo (Vino)"

    # game/zz_spriteobjects.rpy:2258
    old "Ribbon (Yellow)"
    new "Lazo (Amarillo)"

    # game/zz_spriteobjects.rpy:2354
    old "Earrings (Pink Diamond)"
    new "Pendientes (Diamante rosa)"

    # game/zz_spriteobjects.rpy:2430
    old "School Uniform"
    new "Uniforme escolar"

    # game/zz_spriteobjects.rpy:2461
    old "Black Dress"
    new "Vestido negro"

    # game/zz_spriteobjects.rpy:2497
    old "Black and Pink Dress"
    new "Vestido negro y rosa"

    # game/zz_spriteobjects.rpy:2536
    old "School Uniform (Blazerless)"
    new "Uniforme escolar (Sin chaqueta)"

    # game/zz_spriteobjects.rpy:2591
    old "Witch Costume"
    new "Disfraz de bruja"

    # game/zz_spriteobjects.rpy:2627
    old "Neko Costume"
    new "Disfraz de neko"

    # game/zz_spriteobjects.rpy:2679
    old "Lingerie (Spider)"
    new "Lencería (Araña)"

    # game/zz_spriteobjects.rpy:2716
    old "Santa Costume"
    new "Disfraz de Santa"

    # game/zz_spriteobjects.rpy:2752
    old "Lingerie (Santa)"
    new "Lencería (Santa)"

    # game/zz_spriteobjects.rpy:2794
    old "Dress (New Years)"
    new "Vestido (Año Nuevo)"

    # game/zz_spriteobjects.rpy:2832
    old "Sundress (White)"
    new "Vestido de verano (Blanco)"

    # game/zz_spriteobjects.rpy:2864
    old "Lingerie (Pink Lace)"
    new "Lencería (Encaje rosa)"

    # game/zz_spriteobjects.rpy:2926
    old "Shirt (NOU)"
    new "Camiseta (NOU)"
